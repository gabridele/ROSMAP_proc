...
for sub_folder in os.listdir(main_folder_path):
    if not sub_folder.startswith('sub-95638231'):
        continue
    ...
    for inner_folder in os.listdir(sub_folder_path):
        ...
        for filename in os.listdir(inner_folder_path):
            ...
            # Only process files containing '3d_mprage' in the name
            if '3D_MPRAGE' not in filename:
                continue
            ...
            new_filename = f"sub-{sub_digits}_ses-#_acq-BNK20090211_{file_name}_T1w{file_ext}"
    ...
    dl.save(dataset=sub_folder_path, message=f"Renamed 3D_MPRAGE(a) files in {sub_folder}")
